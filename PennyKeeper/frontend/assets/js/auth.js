...
function initLogin() {
    ...
    btn.addEventListener('click', async () => {
        ...
        if (!email || !password) { showAlert(alertEl, 'Omple tots els camps.', 'error'); return; }
        setLoading(btn, true);
    ...
    [emailEl, passEl].forEach(el => {
        el.addEventListener('input', () => { el.classList.remove('is-error'); clearAlert(alertEl); });
...
function initRegister() {
    ...
    btn.addEventListener('click', async () => {
        ...
        if (error) { showAlert(alertEl, error, 'error'); return; }
        setLoading(btn, true);
        const formData = new FormData();
    ...
    confirmEl.addEventListener('keydown', e => { if (e.key === 'Enter') btn.click(); });
}

function validateRegisterForm(username, email, password, passwordConfirm) {
    if (!username || !email || !password || !passwordConfirm) return 'Omple tots els camps.';
    if (username.length < 3)     return 'El nom d\'usuari ha de tenir mínim 3 caràcters.';
    if (!email.includes('@'))    return 'L\'email no és vàlid.';
    if (password.length < 8)     return 'La contrasenya ha de tenir mínim 8 caràcters.';
    if (password !== passwordConfirm) return 'Les contrasenyes no coincideixen.';
    return null;
}
...
async function postToApi(action, formData) {
    try {
        const response = await fetch(
            `${API_BASE}/backend/api/controllers/AuthController.php?action=${action}`,
...
function setLoading(btn, loading) {
    ...
    btn.textContent = loading ? 'Espera...' : (btn.id === 'loginBtn' ? 'Inicia sessió' : 'Crea el compte');
}